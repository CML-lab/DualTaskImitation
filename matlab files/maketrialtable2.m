%generate trial tables for dual-task experiment - larger stimulus set with
%forward and reverse videos (28*2) 
...
%since we only have 4 stimuli, we will have to use all 24 (28) stimuli in each
%  condition and bank on the fact that most people won't be able to
%  memorize the stimuli (i.e., no order effect). 
%To reduce memorization ability, we will show the videos in forward or
%  reverse; this doubles the number of "unique" stimuli without inflating
%  the number of kinematically identical gestures; it also ensures that
%  difficulty is equated across the two conditions.


VidSetOrder = randperm(2);   %we will randomly assign body or trajectory instruction set to get the forward or reverse videos!


for ctx = 1:2 %for 2 context conditions
    
    if VidSetOrder(ctx) == 1
        %use stimulus set 1, posture set 1, trajectory set 1
        Stim = Stim1;
        StimTSub = 0;
        StimTAdd = 0;
        
    else %if VidSetOrder(ctx) == 2
        %use stimulus set 2, posture set 1, trajectory set 2
        Stim = Stim2;
        StimTSub = 32;
        StimTAdd = 48;
    end
    ...
    for istim = 1:length(Stim)
        ...
        for i = 1:length(iset)
            ...
            if stimtype(iset(i),1) == 0 %if we are priming
                stim = Stim{istim}(iset(i),1)-StimTSub;
            else %if we are interfering
                stim = setxor([0:NStimT-1],Stim{istim}(iset(i),1)-StimTSub);
                stim = stim(randperm(length(stim)));
                stim = stim(1);
            end
            ...
            TrTbl{ctx,idts} = [TrTbl{ctx,idts};
                               1 Stim{istim}(iset(i),1) idts stimtype(iset(i),1) stim+StimTAdd respstim+StimTAdd cresp DTstimTime DTwaitTime 500 10000];
...
for ctx = 1:2
    ...
    for istim = 0:3
        ...
        nvid = NVids + istim + StimAdd;
        
        %posture
        stim = randstimp(istim+1);
        ...
        TrTblP{ctx,2} = [TrTblP{ctx,2};
                         1 nvid 2 0 stim+StimTAdd respstim+StimTAdd cresp DTstimTime DTwaitTime 500 10000];
