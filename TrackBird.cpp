...
int TrackBird::InitializeBird(TrackSYSCONFIG *sysconfig)
{
	...
	sysconfig->PosOffset[0] = CALxOFFSET;
	sysconfig->PosOffset[1] = CALyOFFSET;
	sysconfig->PosOffset[2] = CALzOFFSET;
	sysconfig->RotOffset[0] = CALthetaROTANG;
	sysconfig->RotOffset[1] = CALphiROTANG;
	...
	else //trakSTAR initialization code
	{
		...
		SENSOR_CONFIGURATION pSensor[BIRDCOUNT+1];
		...
		//get the system configuration, with information such as how many boards are detected and the nubmer of sensors and transmitters supportable
		errorCode = GetBIRDSystemConfiguration(&sysconfig->systemconfig);
		if(errorCode!=BIRD_ERROR_SUCCESS) 
		{
			std::cerr << "trakSTAR unable to query system configuration." << std::endl;
			didsetup = 0;
		}
		else
			std::cerr << "trakSTAR configuration querried." << std::endl;

		//find the first available transmitter and select it.
		
		USHORT xmtrid;
		for(xmtrid=0;xmtrid<sysconfig->systemconfig.numberTransmitters;xmtrid++)
		{
			errorCode = GetTransmitterConfiguration(xmtrid, &sysconfig->Xmtr[xmtrid]);
		...
		*/
		...
		for (i = 0; i<trakSysConfig.numberSensors; i++)
		{
			...
			if(pSensor[i].attached)
			{
				...
				//set the hemifield
				errorCode = SetSensorParameter(i,HEMISPHERE,&sysconfig->hemifield,sizeof(sysconfig->hemifield));
		...
		std::cerr << "Sensors Active: (" << sysconfig->nBirdsActive << ":"
										 << sysconfig->birdCount << ") ";
		for (i = 0; i<trakSysConfig.numberSensors+1; i++)
		{
			std::cerr << sysconfig->SensorIDs[i] << " " ;
		}
		std::cerr << std::endl;
		...
		else
		{
			...
			xmtrid = -1;
			SetSystemParameter(SELECT_TRANSMITTER, &xmtrid, sizeof(xmtrid));
			CloseBIRDSystem();
		}
...
int TrackBird::GetUpdatedSample(TrackSYSCONFIG *sysconfig, TrackDATAFRAME DataBirdFrame[])
{
	...
	if (sysconfig->trackType == 0)
	{
		...
		if (birdFrameReady(sysconfig->groupID))  //poll to see if a sample is available.  note, all the sensors get updated at the same time.
		{
			...
			for (j = 1; j <= sysconfig->birdCount; j++)
			{
				...
				//Note, the way that the tracker is mounted may require a change of coordinate frames.  For the Kinereach,
				//Tracker X = data Z
				//Tracker Y = data X
				//Tracker Z = data Y
				...
				//for Laurel's system, we will preserve the coordinates as is since we're just recording them.
				...
				DataBirdFrame[j].x = bird_data.position.nX;
				DataBirdFrame[j].y = bird_data.position.nY;
				DataBirdFrame[j].z = bird_data.position.nZ;
				...
				DataBirdFrame[j].x = tmpx*cos(sysconfig->RotOffset[0]) - tmpy*sin(sysconfig->RotOffset[0]);
				DataBirdFrame[j].y = tmpx*sin(sysconfig->RotOffset[0]) + tmpy*cos(sysconfig->RotOffset[0]);
				//DataBirdFrame[j].z = DataBirdFrame[j].z;
				
				DataBirdFrame[j].x += sysconfig->PosOffset[0];
				DataBirdFrame[j].y += sysconfig->PosOffset[1];
				DataBirdFrame[j].z += sysconfig->PosOffset[2];
	...
	else  //trakSTAR system, poll for sample
	{
		...
		if (REPORTRATE > 1 || sysconfig->dosynch)
			errorCode = GetSynchronousRecord(ALL_SENSORS, pbird_data, sizeof(bird_data[0])*BIRDCOUNT);
		else
			errorCode = GetAsynchronousRecord(ALL_SENSORS, pbird_data, sizeof(bird_data[0])*BIRDCOUNT);
		...
		for (j = 1; j <= sysconfig->birdCount; j++)
		{
			...
			if (fabs(bird_data[j-1].time - DataBirdFrame[j].time) > 1e-4 )  //if this is a new sample, save it -- based on the time stamp. note, the danger is that some trackers may be updated and others not, depending on the timing of these calls
			{
				...
				//Note, the way that the tracker is mounted might require a change of coordinate frames.  for the kinereach,
				//Tracker X = data Z
				//Tracker Y = (+/-)data X //negative for minireach, positive for kinereach
				//Tracker Z = data Y
				//For angle, we will record just the angle matrix rather than the azimuth/elevation/roll, to avoid the reference-frame rotation problem

				//for Laurel's system, we will preserve the coordinates as is.
				DataBirdFrame[j].time = bird_data[j-1].time;
				DataBirdFrame[j].etime = bird_data[j-1].time-birds_start;
				DataBirdFrame[j].x = bird_data[j-1].x;
				DataBirdFrame[j].y = bird_data[j-1].y;
